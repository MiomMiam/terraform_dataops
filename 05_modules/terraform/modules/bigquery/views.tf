...
resource "google_bigquery_table" "views" {
  ...
  schema              = file("${var.configuration_folder}/${each.value.dir}/${each.value.content.schema_file}")
  deletion_protection = lookup(each.value.content, "deletion_protection", false)

  view {
    query          = templatefile("${var.configuration_folder}/${each.value.dir}/${each.value.content.query_file}", var.context)
